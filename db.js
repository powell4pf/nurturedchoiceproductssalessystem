...
const Auth = {
  async getSession() {
    ...
    if (response.error) {
      console.error('Critical: RLS Policy Error or Profile Fetch Failure:', response.error.message);
    }

    const profile = response.data;
    const userRole = (profile && profile.role) ? profile.role : 'staff';
    const fullName = (profile && profile.full_name) || (session.user.user_metadata && session.user.user_metadata.full_name) || session.user.email;
